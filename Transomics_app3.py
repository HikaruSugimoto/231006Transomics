...
options = ['Home','A, gene regulatory network (including TF, miRNA, and mRNA) (transcriptome)',
           ...
           "D, metabolic network (including transporter, mRNA, and metabolite) (transcriptome, metabolome (organ or cell) and metabolome (blood or medium))"]
selected_option = st.sidebar.radio('*"TF/miRNA-mRNA" takes several minutes',
                                   options, key="2")
...
if selected_option=="A, gene regulatory network (including TF, miRNA, and mRNA) (transcriptome)":
    if Tran is not None:
        ...
        #mRNA_miRNA
        Tran1=Tran.rename(columns={'ENSMUSG': 'Molecule'})
        Tran2=pd.merge(Tran1, Name, on='Molecule', how='inner')
        Tran3_UP=Tran2[Tran2["FC"]=="UP"]
        Tran3_UP=Tran3_UP.drop_duplicates(subset=['Name'])
        Tran3_Down=Tran2[Tran2["FC"]=="Down"]
        Tran3_Down=Tran3_Down.drop_duplicates(subset=['Name'])
        miRNA_UP=pd.merge(Tran3_UP, miRNA, on='Name', how='inner')
        miRNA_Down=pd.merge(Tran3_Down, miRNA, on='Name', how='inner')
        miRNAmRNA=pd.concat([miRNA_UP,miRNA_Down])
        miRNAmRNA1=miRNAmRNA.iloc[:,[3,2,1]]
        ...
        for i in range (0,len(TF)):
            ...
            P["TF"][i]=TF["TF"][i]
            P["p"][i]=res.pvalue
        P=pd.concat([P,exp],axis=1)
        if Organ=="All":
            P=P.reset_index(drop=True)
        else:
            P=P[P["Var5"]==Organ].reset_index(drop=True)
        q=multi.multipletests(P["p"], alpha=0.05, method='fdr_bh')
...
if selected_option=="D, metabolic network (including transporter, mRNA, and metabolite) (transcriptome, metabolome (organ or cell) and metabolome (blood or medium))":
